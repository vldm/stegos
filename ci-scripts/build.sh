    ...
    *)
    ...
    linux-x64 | macos-x64)
        ...
        for bin in stegos stegosd; do
            files+=("artifacts/$bin$extension")
        done
        tar -czvf release/stegos.tar.gz "${files[@]}"
        ;;
    win-x64)
        for bin in stegos stegosd; do
            zip release/stegos.zip /artifacts/$bin$extension
        done

        for lib in gcc_s_seh-1 lz4 zstd snappy stdc++-6 winpthread-1; do
            zip release/stegos.zip /mingw64/bin/lib$lib.dll
        done
        ;;
    android-x64)
        tar -czvf release/libstegos.tar.gz artifacts/$dylib $ANDROID_SDK_DIR/ndk-bundle/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/x86_64-linux-android/libc++_shared.so

        ;;
    android-aarch64)
        tar -czvf release/libstegos.tar.gz artifacts/$dylib$ANDROID_SDK_DIR/ndk-bundle/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/aarch64-linux-android/libc++_shared.so
        ;;
    *)
        echo 2>&1 "Unknown platform: $1"
        exit 1
